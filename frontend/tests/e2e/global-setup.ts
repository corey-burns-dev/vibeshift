...
async function signup(
	...
	while (attempt < maxAttempts) {
		...
		if (res.status() === 429) {
			const waitMs = Math.min(10000, 500 * 2 ** attempt);
			// eslint-disable-next-line no-await-in-loop
			await new Promise((resolve) => setTimeout(resolve, waitMs));
			continue;
		}

		lastErr = `signup failed (${res.status()}): ${await res.text()}`;
		break;
	}

	if (lastErr === null) {
		lastErr = `rate limited after ${maxAttempts} attempts`;
	}
...
export default async function globalSetup() {
	...
	try {
		// Try to reuse seeded accounts if available to avoid signup rate limits
		let user = await tryLogin(api, "test@example.com", "password123");
		// Prefer a known dev admin if present
		let admin = await tryLogin(api, "admin@admin.com", "admin");

		if (!user) {
			user = await signup(api, "e2euser");
		}
		if (!admin) {
			admin = await signup(api, "e2eadmin");
		}
